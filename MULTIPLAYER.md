...
apps/client                              apps/server
  ...
  NetworkSession  инпуты 60/с ──WS──▶     Match: World (тот же код, что в игре), 60 тиков/с
               ◀── снапшоты 60/с ──       users.json на диске (рейтинг)
  "Sign in with GitHub/Google" ──HTTP──▶  /auth/... (OAuth)
```

- **Сервер решает всё.** Клиент шлёт только нажатые кнопки (`PlayerInput`), сервер крутит `World` и 60 раз в секунду рассылает снимок мира (~35 КБ/с на игрока после сжатия).
...
- **Свои выстрелы — сразу.** Пули, удар мечом и луч рисуются в момент нажатия (клиент считает их по тем же правилам, что сервер), серверная копия своей пули скрыта. Цифры урона от своих попаданий показываются, как только сервер их подтвердил.
- **«Мультик» поверх решений сервера** (`session/Anticipation.ts`). Сервер по-прежнему решает всё, а клиент сразу показывает то, что он почти наверняка решит: цифры урона, просадку HP и смерть моба от твоей пули, меча или дробовика; срезание пуль мечом; подбор опыта (шарики летят к тебе настоящему); щит и выстрел дробовика. Если сервер не согласился (редко), картинка тихо возвращается: моб «оживает», шарик появляется снова. Жульничать это не позволяет — клиент ничего не решает.
- **Чужие пули показаны «в твоём времени».** Пуля летит по прямой, поэтому клиент рисует её там, где она будет, когда до сервера дойдёт твоё действие (задержка отрисовки + полпинга). Уклонение рывком, срезание мечом и попадания по тебе совпадают с тем, что видишь.
- **Всё остальное рисуется чуть в прошлом.** Между двумя снимками идёт плавная интерполяция: мобы, пули и соперник двигаются ровно даже при скачущем пинге. Задержка подстраивается под сеть: на ровном соединении ~35–50 мс, при рывках растёт до 150 мс.
